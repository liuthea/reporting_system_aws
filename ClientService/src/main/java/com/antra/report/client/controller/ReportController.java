...
public class ReportController {
    ...
    public ResponseEntity<GeneralResponse> editReportDirectly(@PathVariable String reqId, @RequestBody @Validated ReportRequest request) throws ExecutionException, InterruptedException {
        log.info("Got Request to edit report - {}", request);
        request.setDescription(String.join(" - ", "Sync", request.getDescription()));
        return ResponseEntity.ok(new GeneralResponse(reportService.updateReport(reqId,request)));
    }

    @DeleteMapping("/report/{reqId}") //delete request, pdf, excel
